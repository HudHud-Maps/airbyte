...
class IcebergUtil(
     ...
     *
     * For a multi-level namespace (e.g. `poi.bronze` -> `[poi, bronze]`), every ancestor level is
     * created parent-first (`[poi]` then `[poi, bronze]`), since REST catalogs require the parent
     * namespace to exist before a child can be created. Single-level namespaces execute exactly one
     * iteration, preserving the previous behavior.
     */
    fun createNamespace(streamDescriptor: DestinationStream.Descriptor, catalog: Catalog) {
        ...
        val levels = tableIdentifier.namespace().levels()
        synchronized(tableIdentifier.namespace()) {
            if (catalog is SupportsNamespaces) {
                for (depth in 1..levels.size) {
                    ...
                    if (!catalog.namespaceExists(ancestor)) {
                        try {
                            ...
                            // This exception occurs when multiple threads attempt to write to the
                            // same namespace in parallel.
                            // One thread may create the namespace successfully, causing the other
                            // threads to encounter this exception
